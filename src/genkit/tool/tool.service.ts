...
export class ToolService implements OnModuleInit {
    ...
    async onModuleInit() {
        // Wait until GenkitService has finished initializing
        ...
        this.addRecipeTool = ai.defineTool(
            ...
            async ({ title, description, items }) => {
                ...
                return {
                    id: recipe.id,
                    message:
                        `Recipe "${recipe.title}" with ${recipe._count.items} item(s) added successfully!`,
                };
            },
        );

        // View Recipes Tool
        const viewRecipesTool = ai.defineTool({
            name: "viewRecipes",
